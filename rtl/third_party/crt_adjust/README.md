...
fits your CRT perfectly — **without the CRT ever losing lock**, and without
...
## NEW · CRT V-Size (experimental) — vertical stretch / shrink

[`rtl/crt_vsize.sv`](rtl/crt_vsize.sv) is a **separate, self-contained
companion stage** — existing `crt_adjust` integrations are not affected in
any way. First shipped in
[NeoGeo_MiSTer_Streamer](https://github.com/rmonic79/NeoGeo_MiSTer_Streamer),
where it is HW-validated on an arcade cabinet chassis, a 9" PVM and a Barco
RGB monitor; it stays marked *experimental* while community feedback on more
chassis comes in.

Changing the vertical size of a CRT picture always has a price — the module
offers **two exclusive modes** that pay it in different currencies:

| | **PVM mode** — line retimer | **Cabinet mode** — photometric |
|---|---|---|
| Mechanism | retimes the total line count per frame: every line stays **unique** — zero artifacts, the vertical twin of H-Size | timing stays **100% native**: each source line's light is redistributed onto the fixed scanline grid (gamma-correct 2-tap in linear light, energy-preserving) — what the chassis V-size pot does, computed |
| HSync | moves ~0.4 % per line | untouched to the Hz — **losing sync is physically impossible** |
| Use on | broadcast monitors with a wide H-lock (PVM/BVM, Barco RGB) | arcade cabinet chassis, consumer TVs, anything that refuses frequency changes |
| Trade-off | the monitor must follow the frequency | a very slight, uniform vertical softness — no banding, no doubled lines, no shimmer |

Integration notes: it sits **before** `crt_adjust` (`native stream →
crt_vsize → crt_adjust → OSD`), is fully **self-measuring** (no per-core
timing parameters, follows NTSC/PAL switches by itself), needs a
clk-to-pixel ratio ≥ 8, and costs ~`RING_LINES × LINE_PX × 24` bit of BRAM
(46 lines ≈ 17 M10K). On large enlargements Cabinet mode micro-delays the
output VSync so the extra rows escape the vertical retrace and the picture
recenters itself.

**Start here to integrate it:**
* [`docs/vsize-integration.md`](docs/vsize-integration.md) — the complete
  integration guide: chain placement, requirements, ports, OSD strings,
  decode, instantiation, the `vb` rule, bring-up checklist
* [`examples/vsize_integration_snippet.v`](examples/vsize_integration_snippet.v)
  — the wiring fragments, verbatim from the shipping NeoGeo integration

The physics, the design decisions and the dead ends worth not retrying are
documented in [`docs/v-size-design.md`](docs/v-size-design.md) and
[`docs/v-size-desync-roadmap.md`](docs/v-size-desync-roadmap.md).
...
> **Field note (2026-08-05, code-verified on the current MiSTer framework —
> NeoGeo_MiSTer sys, first sys-side integration in
> [NeoGeo_MiSTer_Streamer](https://github.com/rmonic79/NeoGeo_MiSTer_Streamer)):**
> the HDMI scaler taps the slot-line stream **above** the insertion point
> (`ce_hpix/hr_out/hhs_fix/hvs_fix/hde_emu` are assigned from `vga_*_sl`), so
> **all three controls — H-Size, H-Position AND V-Shift — stay on the analog
> branch only**: the module's shifted syncs never reach the HDMI scaler. The
> "H-Pos/V-Shift reach HDMI" caveat above does not apply to this framework
> version. The only HDMI mode that follows the adjust is `direct_video`
> (it taps post-OSD), which is itself a CRT-style output, so that is the
> desired behavior. Two integration rules confirmed:
> 1. `vb_in` MUST be the core's TRUE vertical blank, forwarded from the emu
>    (e.g. an added `CRT_VBL` port). Feeding the combined blank (`~vga_de_sl`)
>    like the legacy `analog_hsize` glue did gives a BLACK SCREEN with this
>    engine: at every HSync rise the combined blank is 1, so `vb_active`
>    latches 1 on every line and `pass_q` never opens.
> 2. Gate `active` off in the core when the scandoubler is enabled — the
>    read-rate base assumes the native (15 kHz) pixel-CE ratio.
> HW test pending; engine is line-for-line the HW-proven core-side one.
...
│   ├── crt_vsize.sv           CRT V-Size — vertical, PVM/Cabinet (experimental)
│   └── old/
...
│   ├── vsize-integration.md       V-Size integration guide (start here)
│   ├── v-size-design.md           V-Size design decisions & physics
│   └── v-size-desync-roadmap.md   V-Size anti-desync research notes
└── examples/
    ├── core_side_snippet.v        Reference glue for core-side (your core .sv)
    ├── sys_top_snippet.v          Reference glue for sys-side (sys_top.v)
    └── vsize_integration_snippet.v  V-Size chain glue (from NeoGeo Streamer)
```
...
The **CRT Adjust** module (H-Size + H-Position + V-Shift, with the parametric
`HPOS_MODE` content-shift / sync-shift design) is by **rmonic79**.
