// SIM-ONLY COPY of rtl/third_party/crt_adjust/rtl/crt_vsize.sv with the line_base() calls
// inlined: Verilator 5.032 dies with an internal error (V3Delayed.cpp "Multiple Write refs on
// LHS of NBA") on a function call inside the index of a non-blocking assignment. The shipped
// file is untouched; regenerate this one with the sed in the Makefile after any upstream bump.
//============================================================================
...
//============================================================================
//
//  *** MODIFIED for Arcade-NMK16_MiSTer (2026-09-18) -- see deps.lock. ***
//  One register (ring_q2) was added between the ring RAM's read mux and
//  the Cabinet-mode square multiplier, and that pipeline's phases moved
//  one clock later (squares at p_cnt 4/5, blend 6). Reason: the ring is
//  42 M10K blocks here (46 x 384 x 24), and RAM-output-mux -> 8x8
//  multiply in ONE clock was -0.86 ns at 112 MHz (NMK16_Macross2) and
//  +0.04 ns at 96 MHz. The integer square root was then split from two
...
module crt_vsize #(
    ...
    always @(posedge clk) begin
        ...
        if (pxl_cen & de_in) begin
            ring[((wl) * LINE_PX) + wpx] <= {r_in, g_in, b_in};
    ...
    reg  [23:0]    ring_q;
    reg  [23:0]    ring_q2;     // NMK16: one more stage for the Cabinet squares (see header)
    initial begin rd_addr = 0; rd_addr_t = 0; rd_addr_m = 0; ring_q = 0; ring_q2 = 0; end
    always @(posedge clk) begin
    ...
    //  (causalita`) -> ricentrare con V-Shift. Richiede clk/pixel >= 11 (NMK16: >= 8 upstream).
    ...
    // ---- radice quadrata intera 16->8 bit, spezzata in QUATTRO stadi ----
    // (NMK16: two compare-subtract steps per clock instead of four; the
    //  same restoring algorithm, {x, res} carried between stages)
    function [31:0] isqrt_step2(input [31:0] xr, input [15:0] b1, input [15:0] b2);
        reg [15:0] x, res;
        begin
            x = xr[31:16]; res = xr[15:0];
            if (x >= res + b1) begin x = x - (res + b1); res = (res >> 1) + b1; end else res = res >> 1;
            if (x >= res + b2) begin x = x - (res + b2); res = (res >> 1) + b2; end else res = res >> 1;
            isqrt_step2 = {x, res};
        end
    endfunction
    function [7:0] isqrt_last(input [31:0] xr);   // final two steps, result byte
        reg [31:0] t;
        begin
            t = isqrt_step2(xr, 16'h0004, 16'h0001);
            isqrt_last = t[7:0];
        end
    endfunction
    ...
    reg  [31:0] t_h2_r, t_h2_g, t_h2_b;   // NMK16: sqrt stages 2..3
    reg  [31:0] t_h3_r, t_h3_g, t_h3_b;
    reg  [23:0] t_out;
    reg         t_out_v;
    initial begin
        ...
        t_h2_r=0; t_h2_g=0; t_h2_b=0; t_h3_r=0; t_h3_g=0; t_h3_b=0;
        t_out=0; t_out_v=0;
    end
    ...
    always @(posedge clk) begin
        ...
        // dato B durante la fase 4; NMK16: quadrati un ciclo dopo, da ring_q2,
        // cosi` il mux della RAM e il moltiplicatore non stanno in un ciclo):
        case (p_cnt)
            5'd0: rd_addr_t <= ((t_slotA) * LINE_PX) + t_px_l;
            5'd1: rd_addr_t <= ((t_slotB) * LINE_PX) + t_px_l;
            5'd4: begin
                t_sqa_r <= ring_q2[23:16] * ring_q2[23:16];
                t_sqa_g <= ring_q2[15:8]  * ring_q2[15:8];
                t_sqa_b <= ring_q2[7:0]   * ring_q2[7:0];
            end
            5'd5: begin
                t_sqb_r <= ring_q2[23:16] * ring_q2[23:16];
                t_sqb_g <= ring_q2[15:8]  * ring_q2[15:8];
                t_sqb_b <= ring_q2[7:0]   * ring_q2[7:0];
            end
            5'd6: begin
                t_bl_r <= t_mix_r[23:8];
                t_bl_g <= t_mix_g[23:8];
            ...
            5'd7: begin
                t_h1_r <= isqrt_step2({t_bl_r, 16'd0}, 16'h4000, 16'h1000);
                t_h1_g <= isqrt_step2({t_bl_g, 16'd0}, 16'h4000, 16'h1000);
                t_h1_b <= isqrt_step2({t_bl_b, 16'd0}, 16'h4000, 16'h1000);
            end
            5'd8: begin
                t_h2_r <= isqrt_step2(t_h1_r, 16'h0400, 16'h0100);
                t_h2_g <= isqrt_step2(t_h1_g, 16'h0400, 16'h0100);
                t_h2_b <= isqrt_step2(t_h1_b, 16'h0400, 16'h0100);
            end
            5'd9: begin
                t_h3_r <= isqrt_step2(t_h2_r, 16'h0040, 16'h0010);
                t_h3_g <= isqrt_step2(t_h2_g, 16'h0040, 16'h0010);
                t_h3_b <= isqrt_step2(t_h2_b, 16'h0040, 16'h0010);
            end
            5'd10: begin
                t_out   <= {isqrt_last(t_h3_r), isqrt_last(t_h3_g), isqrt_last(t_h3_b)};
                t_out_v <= t_fetch;
            end
            default: ;
        endcase
